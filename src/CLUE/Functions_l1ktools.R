...
#Function modified by Kevin original function(append.dim), actual name (append.dim_modif)
#The original function did not save the file, because they use a function path.join which doest not belongs to R
#The function write.gctx calls to this functions, this call is also modified
append.dim_modif <- function(ofile, mat, extension="gct") {
  nc <- ncol(mat)
  nr <- nrow(mat)
...
write.gct <- function(ds, ofile, precision=4, appenddim=T, ver=3) {
  ...
  if (appenddim) ofile <- append.dim_modif(ofile, ds@mat, extension="gct")
...
write.gctx <- function(ds, ofile, appenddim=T, compression_level=6, matrix_only=F) {
  if (appenddim) {
    ofile <- append.dim_modif(ofile, ds@mat, extension="gctx")  
  }
